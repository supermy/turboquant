...
fn test_siftsmall_recall_and_qps() {
    ...
    if !data_dir.exists() {
        println!("跳过测试: 数据目录不存在 (设置 SIFT_DATA_DIR 环境变量)");
        return;
    }
    ...
    println!(
        ...
        d,
        nb * d * 4 / 1024,
        nb as f64 * d as f64 * 4.0 / 1024.0 / 1024.0
    );
    ...
    for r in &results {
        ...
        println!(
            "{:<25}: Recall={:>5.1}分  Speed={:>5.1}分  Storage={:>5.1}分  综合={:>6.1}",
    ...
    println!(
        "  - 低延迟需求 -> {} ({} QPS)",
        best_speed.name, best_speed.qps as i64
    );
}
