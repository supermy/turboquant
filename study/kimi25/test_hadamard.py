...
def test_rotation_properties():
    ...
    # Hadamard旋转
    y_hadamard = hadamard_rotation_numpy(x, seed=42)
...
def test_quantization_effect():
    ...
    # Hadamard旋转后量化
    y = hadamard_rotation_numpy(x, seed=42)
    y_quant, _ = quantize(y, bits=4)
    # 反旋转（近似）
    x_recon = hadamard_rotation_numpy(y_quant, seed=42)  # 用相同种子
